...
export default function AnkletsPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.log('🔄 Fetching anklets products...')
      ...
      if (data.success) {
        ...
        // Filter to show only anklets products
        const ankletsProducts = allProducts.filter((product: Product) => {
          const isAnklets = product.name.toLowerCase().includes('anklet') ||
                           product.name.toLowerCase().includes('payal') ||
                           product.name.toLowerCase().includes('ankle') ||
                           product.name.toLowerCase().includes('ankle chain') ||
                           product.subcategory === 'anklets' ||
                           (product.subcategory === 'jewelry' && product.name.toLowerCase().includes('anklet'))
          
          console.log(`🔍 Product: ${product.name}, Is Anklets: ${isAnklets}`)
          return isAnklets
        })
        
        console.log(`✅ Found ${ankletsProducts.length} anklets products from ${allProducts.length} total products`)
        setProducts(ankletsProducts)
      } else {
      ...
      <div className="min-h-screen bg-gradient-to-b from-indian-gold/5 via-indian-saffron/5 to-indian-crimson/5">
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-center">
            ...
            <p className="text-indian-maroon font-medium">Loading Anklets...</p>
          </div>
        </div>
        <Footer />
      </div>
      ...
      <div className="relative bg-gradient-to-br from-indian-gold/20 via-indian-saffron/20 to-indian-crimson/20 py-16 lg:py-24">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl lg:text-6xl font-bold text-indian-maroon mb-6">
            <span className="block">Anklets</span>
            <span className="block text-3xl lg:text-5xl text-indian-gold">Payal & Ankle Chains</span>
          </h1>
          <p className="text-xl text-indian-crimson max-w-3xl mx-auto leading-relaxed">
            Discover our exquisite collection of traditional Indian anklets, payals, and ankle chains, 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-indian-crimson">
            {products.length} beautiful anklets to choose from
          </p>
        </div>
        ...
        {products.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-2xl font-semibold text-indian-maroon mb-2">No Anklets Found</h3>
            <p className="text-indian-crimson mb-6">We're currently updating our anklets collection. Please check back soon!</p>
            <button 
              onClick={fetchProducts}
