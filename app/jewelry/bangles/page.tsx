...
export default function BanglesPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.log('🔄 Fetching bangles products...')
      ...
      if (data.success) {
        ...
        // Filter to show only bangles products
        const banglesProducts = allProducts.filter((product: Product) => {
          const isBangles = product.name.toLowerCase().includes('bangle') ||
                           product.name.toLowerCase().includes('kada') ||
                           product.subcategory === 'bangles' ||
                           (product.subcategory === 'jewelry' && product.name.toLowerCase().includes('bangle'))
          
          console.log(`🔍 Product: ${product.name}, Is Bangles: ${isBangles}`)
          return isBangles
        })
        
        console.log(`✅ Found ${banglesProducts.length} bangles products from ${allProducts.length} total products`)
        setProducts(banglesProducts)
      } else {
      ...
      <div className="min-h-screen bg-gradient-to-b from-indian-gold/5 via-indian-saffron/5 to-indian-crimson/5">
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-center">
            ...
            <p className="text-indian-maroon font-medium">Loading Bangles...</p>
          </div>
        </div>
        <Footer />
      </div>
      ...
      <div className="relative bg-gradient-to-br from-indian-gold/20 via-indian-saffron/20 to-indian-crimson/20 py-16 lg:py-24">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl lg:text-6xl font-bold text-indian-maroon mb-6">
            <span className="block">Traditional Bangles</span>
            <span className="block text-3xl lg:text-5xl text-indian-gold">Kadas & Bracelets</span>
          </h1>
          <p className="text-xl text-indian-crimson max-w-3xl mx-auto leading-relaxed">
            Discover our exquisite collection of traditional Indian bangles and kadas, 
            handcrafted with authentic designs and premium materials.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-indian-crimson">
            {products.length} beautiful bangles to choose from
          </p>
        </div>
        ...
        {products.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-2xl font-semibold text-indian-maroon mb-2">No Bangles Found</h3>
            <p className="text-indian-crimson mb-6">We're currently updating our bangles collection. Please check back soon!</p>
            <button 
              onClick={fetchProducts}
