          ...
          <Link href={`/products/${product._id || product.id}`} className="block">
            <div className="relative mb-4 lg:mb-6">
              ...
              <Button
                size="icon"
                variant="ghost"
                className={`absolute top-2 lg:top-3 right-2 lg:right-3 bg-white/90 hover:bg-white shadow-lg border border-amber-200 w-8 h-8 lg:w-10 lg:h-10 ${
              ...
              >
                <Heart className={`h-3 lg:h-5 w-3 lg:w-5 ${
                  isInWishlist(product._id || product.id || '') ? 'fill-current' : ''
                }`} />
              </Button>
              {/* Category Badge */}
